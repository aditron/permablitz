...
class AlxPbTabs extends WP_Widget {

/*  Constructor
/* ------------------------------------ */

public function __construct() {
		parent::__construct( false, 'AlxPbTabs', array('description' => 'List posts, comments, and/or tags with or without tabs.', 'classname' => 'widget_alx_tabs') );
	}
...
?>
	<div class="alx-tabs-container">
		...
		<?php if($instance['recent_enable']) { // Recent posts enabled? ?>
			...
			<?php $latest = get_transient("latest_posts");
	// $latest = false;
if ($latest === false ) {
ob_start(); ?>
			...
			<ul id="tab-recent" class="alx-tab group <?php if($instance['recent_thumbs']) { echo 'thumbs-enabled'; } ?>">
				...
				<li>
					...
					<div class="tab-item-inner group">
						...
						<?php if($instance['tabs_date']) { ?><p class="tab-item-date"><?php the_date(); ?></p><?php } ?>
					</div>
...
<?php $latest = ob_get_clean(); 
set_transient("latest_posts", $latest, 180 * MINUTE_IN_SECONDS);
}
echo $latest;
?>
		<?php } ?>
...
p.ID, p.post_type, p.post_title, SUM(v.pageviews) AS 'pageviews'
FROM pbz_popularpostssummary v LEFT JOIN pbz_posts  p ON v.postid = p.ID
AND v.last_viewed > DATE_SUB(NOW(), INTERVAL 1 WEEK)
WHERE 1 = 1
AND p.post_type IN ('post', 'event')
...
LIMIT 5");
			...
			$pop_output .= '">';
				$df = get_option('date_format');
					...
					$pop_output .= '<div class="tab-item-thumbnail">';
					$pop_output .= '<a href="'. get_permalink($post->ID) . '" title="' . get_the_title() . '">';
						if ( has_post_thumbnail() ):
							$pop_output .= get_the_post_thumbnail($post->ID, 'thumb-small');
							else:
							$pop_output .= '<img src="' . get_template_directory_uri() . '/img/thumb-small.png" alt="' . get_the_title() . '" />';
							endif; 
							if ( has_post_format('video') && !is_sticky() ) $pop_output .= '<span class="thumb-icon small"><i class="fa fa-play"></i></span>';
							if ( has_post_format('audio') && !is_sticky() ) $pop_output .= '<span class="thumb-icon small"><i class="fa fa-volume-up"></i></span>';
							if ( is_sticky() ) $pop_output .= '<span class="thumb-icon small"><i class="fa fa-star"></i></span>';
					$pop_output .= '</a>';
					$pop_output .= '</div>';
					}
					
					$pop_output .= '<div class="tab-item-inner group">';
					if($instance['tabs_category']) { 
						$pop_output .= '<p class="tab-item-category">' . get_the_category_list( '/' ) . '</p>'; }
						$pop_output .= '<p class="tab-item-title"><a href="' . get_permalink($post->ID). '" rel="bookmark" title="' . get_the_title() . '">' . get_the_title() . '</a></p>';
						if($instance['tabs_date']) { $pop_output .= '<p class="tab-item-date">' . get_the_date($df, $post->ID) . '</p>'; }
					$pop_output .= '</div>';
				...
				set_transient("popular_posts", $pop_output, 180 * MINUTE_IN_SECONDS);

				$popular_posts = $pop_output;
		} 
		echo $popular_posts;
		?>
				<?php wp_reset_postdata(); ?>
				...
				<li>
					<?php 
						...
						if ($tag_cloud === false) {
							...
							set_transient("tag_cloud", $tag_cloud, 0);
						}
						echo $tag_cloud;						
					?>
				</li>
