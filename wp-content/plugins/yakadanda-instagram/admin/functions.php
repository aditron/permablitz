...
function yinstagram_admin_notice() {
  ...
  $class = 'updated yinstagram-notice';
  $dismiss_button = '<p class="yinstagram-close"><a id="yinstagram-dismiss" href="#">' . __('Close', 'yakadanda-instagram') . '</a></p>';
  
  if (version_compare( $wp_version, '4.4', '>=' )) {
    $class .= ' settings-error notice is-dismissible';
    $dismiss_button = null;
  }
...
function yinstagram_get_wrap_admin_title($admin_page_title) {
  $wp_version = get_bloginfo( 'version' );
  ...
  return $before . $admin_page_title . $after;
}
